...
#    print(X,X.shape)
#    print(y,y.shape)
    arr = model.train(X,y_scaled,0.1,50,"batch")
    ...
    W_mesh = np.concatenate((np.ones(r.ravel().shape).reshape(-1,1)*1.8, r.ravel().reshape(-1,1),s.ravel().reshape(-1,1)), axis=1)
    ...
    # ax = plt.axes(projection='3d')
    # W_arr = np.array(model.W_arr)
    # ax.scatter(W_arr[:,1], W_arr[:,2], arr,cmap='binary')
    # plt.show()