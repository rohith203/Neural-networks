...
from scipy.cluster.vq import kmeans2 as KMeans, whiten
...
def train(X_train, y_train):
    ...
    if kernel==gaussian:
        while True:
            f = True
            means, assignments = KMeans(X_train, k, iter=max_iter)
            for i in range(k):
                if X_train[(assignments==i)].shape[0]<=1:
                    f = False
                    break
            if f==False: pass
            else: break
    ...
    for i in range(k):
        ...
        t = (1/temp.shape[0])*np.sum(np.linalg.norm(temp-means[i]))
        stds.append(t)
    stds = np.array(stds)
    # print(stds)
